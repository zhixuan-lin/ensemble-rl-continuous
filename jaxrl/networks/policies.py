...
class NormalTanhPolicy(nn.Module):
    ...
    def __call__(self,
        ...
        if self.fn:
            outputs = outputs / (jnp.linalg.norm(outputs, ord=2, axis=-1, keepdims=True) + 1e-6)
...
class NormalTanhPolicyParams(nn.Module):
    hidden_dims: Sequence[int]
    action_dim: int
    state_dependent_std: bool = True
    dropout_rate: Optional[float] = None
    ...
    def __call__(self,
                 observations: jnp.ndarray,
                 temperature: float = 1.0,
                 training: bool = False) -> Tuple[jnp.array, jnp.array]:
        outputs = MLP(self.hidden_dims,
                      activate_final=True,
                      dropout_rate=self.dropout_rate)(observations,
                                                      training=training)
...
class EnsembleNormalTanhPolicy(nn.Module):
    hidden_dims: Sequence[int]
    action_dim: int
    state_dependent_std: bool = True
    dropout_rate: Optional[float] = None
    ...
    def __call__(self,
                 ...
                 temperature: float = 1.0,
                 training: bool = False) -> tfd.Distribution:
        VmapPolicy = nn.vmap(NormalTanhPolicyParams,
                             variable_axes={'params': 0},
                             split_rngs={'params': True},
                             in_axes=None if self.share_batch else (0, None, None),
                             out_axes=(0, 0),
                             axis_size=self.num_pis)
        means, log_stds = VmapPolicy(self.hidden_dims,
                                     ...
                                     self.log_std_min,
                                     self.log_std_max,
                                     self.tanh_squash_distribution
                                     )(observations, temperature, training)
        base_dist = tfd.MultivariateNormalDiag(loc=means,
                                               scale_diag=jnp.exp(log_stds) *
                                               temperature)
        if self.tanh_squash_distribution:
...
parallel_sample = jax.jit(
    jax.vmap(
        sample_actions,
        # rng, actor_params, observations
        in_axes=(0, None, 0, 0, None, None)
    ),
    static_argnames=('actor_def', 'distribution')
)
